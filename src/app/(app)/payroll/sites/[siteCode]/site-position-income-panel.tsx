...
// Same "own slice of data as a prop, refresh() after mutation" pattern as
// LeaveTenureTierPanel — no fetch-on-mount, the expand click is the event.
export default function SitePositionIncomePanel({
  sitePositionId,
  initialIncomes,
  incomeTypes,
  canSave,
  ...
  async function refresh() {
    const res = await fetch(`/api/site-position-incomes?sitePositionId=${sitePositionId}`);
  ...
  async function handleCreate() {
    ...
    setPending(true);
    try {
      const res = await fetch("/api/site-position-incomes", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sitePositionId, incomeCode: form.incomeCode, amount: Number(form.amount), rateBasis: form.rateBasis }),
      });
  ...
  function startEdit(r: IncomeRow) {
    setEditingId(r.SitePositionIncomeID);
    setEditForm({ amount: r.Amount, rateBasis: r.RateBasis });
  }

  async function saveEdit(id: number) {
    setMessage(null);
    const res = await fetch(`/api/site-position-incomes/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async function remove(r: IncomeRow) {
    ...
    setMessage(null);
    const res = await fetch(`/api/site-position-incomes/${r.SitePositionIncomeID}`, { method: "DELETE" });
    const body = await res.json().catch(() => ({}));
    if (!res.ok) {
      setMessage(body.message || body.error);
      return;
  ...
  return (
    <div className="rounded-md border border-gray-200 bg-gray-50 p-3">
      <p className="mb-2 text-xs font-medium text-gray-500">รายได้ของตำแหน่งนี้ที่หน่วยงานนี้</p>
      <table className="w-full border-collapse text-sm">
        ...
        <tbody>
          {rows.map((r) => {
            const isEditing = editingId === r.SitePositionIncomeID;
            return (
              <tr key={r.SitePositionIncomeID} className="border-t border-gray-100">
                  ...
                  <td className="py-1 text-right">
                    {isEditing ? (
                      <div className="flex justify-end gap-2">
                        <button onClick={() => saveEdit(r.SitePositionIncomeID)} className="text-gray-900 hover:underline">
                          บันทึก
                        </button>
