...
import { TableView, BarChartView, PieChartView, LineChartView } from "./charts";
...
const METRICS = [
  { key: "age", label: "อายุพนักงาน (จำนวนคนต่อช่วงอายุ)", needsPeriod: false },
  { key: "headcount-by-site", label: "จำนวนคนในหน่วยงาน (Site)", needsPeriod: false },
  { key: "cost-by-site", label: "ค่าใช้จ่ายรวมตามหน่วยงาน (% เทียบทั้งหมด)", needsPeriod: true },
] as const;
...
export default function DashboardView({
  ...
  async function load() {
    ...
    setPending(true);
    try {
      const params = new URLSearchParams({ metric, employeeType });
      ...
      // apiSuccess() serializes the DashboardResult ({data, unit}) as the
      // top-level JSON body directly — no extra "data" wrapper.
      setResult(body as DashboardResult);
    } finally {
      setPending(false);
    }
  }
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end gap-3 rounded border border-gray-200 p-3">
        <div>
          ...
          <select value={metric} onChange={(e) => selectMetric(e.target.value as MetricKey)} className={`${selectCls} w-80`}>
            {METRICS.map((m) => (
      ...
      <div className="rounded border border-gray-200 p-4">
        {viewMode === "table" && <TableView data={result.data} unit={result.unit} />}
        {viewMode === "bar" && <BarChartView data={result.data} unit={result.unit} />}
        {viewMode === "pie" && <PieChartView data={result.data} unit={result.unit} />}
        {viewMode === "line" && <LineChartView data={result.data} unit={result.unit} />}
      </div>
    </div>
  );
}
