...
async function confirmDeleteStockIssue(label: string): Promise<boolean> {
...
function label(r: HeaderRow) {
  return `${r.DocumentNo ? `#${r.DocumentNo}` : `(เอกสาร #${r.IssueHeaderID})`}`;
}

export default function StockIssueListView({
  initialRows,
  warehouses,
  employees,
  canSave,
  ...
  async function refresh() {
    const res = await fetch("/api/inventory/stock-issues");
    if (res.ok) setRows(await res.json());
  }

  async function handleDelete(r: HeaderRow) {
    if (!(await confirmDeleteStockIssue(label(r)))) return;
    setMessage(null);
    const res = await fetch(`/api/inventory/stock-issues/${r.IssueHeaderID}`, { method: "DELETE" });
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="border-b border-gray-200 bg-gray-50 text-left text-gray-500">
            <tr>
              ...
              <th className="px-3 py-2 font-medium">คลังที่เบิกจำหน่าย</th>
          ...
          <tbody>
            {rows.map((r) => {
              const total = r.Details.filter((d) => !d.IsWelfare).reduce((sum, d) => sum + Number(d.Amount), 0);
              return (
                <tr key={r.IssueHeaderID} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-3 py-2">
                    <Link href={`/inventory/transactions/issue/${r.IssueHeaderID}`} className="text-gray-900 hover:underline">
                      {r.DocumentNo ?? `#${r.IssueHeaderID}`}
                    </Link>
                  </td>
