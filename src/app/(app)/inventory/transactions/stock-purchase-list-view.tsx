...
async function confirmDeleteStockPurchase(label: string): Promise<boolean> {
...
interface HeaderRow {
  ...
  Remark: string | null;
  Status: string;
  Details: { PurchaseDetailID: number; Amount: string }[];
}
...
function label(r: HeaderRow) {
  return `${r.DocumentNo ? `#${r.DocumentNo}` : `(เอกสาร #${r.PurchaseHeaderID})`}`;
}

export default function StockPurchaseListView({
  initialRows,
  warehouses,
  suppliers,
  canSave,
  ...
  async function refresh() {
    const res = await fetch("/api/inventory/stock-purchases");
    if (res.ok) setRows(await res.json());
  }

  async function handleDelete(r: HeaderRow) {
    if (!(await confirmDeleteStockPurchase(label(r)))) return;
    setMessage(null);
    const res = await fetch(`/api/inventory/stock-purchases/${r.PurchaseHeaderID}`, { method: "DELETE" });
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="border-b border-gray-200 bg-gray-50 text-left text-gray-500">
            <tr>
              ...
              <th className="px-3 py-2 font-medium">คลังที่รับเข้า</th>
              <th className="px-3 py-2 font-medium">ผู้จำหน่าย</th>
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.PurchaseHeaderID} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-3 py-2">
                    <Link href={`/inventory/transactions/purchase/${r.PurchaseHeaderID}`} className="text-gray-900 hover:underline">
                      {r.DocumentNo ?? `#${r.PurchaseHeaderID}`}
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-2 text-gray-500">
                    {r.SupplierCode} — {r.Supplier.SupplierName}
                  </td>
                  <td className="px-3 py-2 text-gray-500">{r.SupplierDeliveryNo ?? "-"}</td>
      ...
      {canSave && (
        <div className="flex flex-wrap items-end gap-2 rounded-lg border border-dashed border-gray-300 p-3">
          ...
          <label className="flex flex-col gap-1 text-xs text-gray-500">
            ...
            <div className="w-56">
              <SearchableSelect
                ...
                onChange={(code) => setForm({ ...form, supplierCode: code })}
                options={suppliers.map((s) => ({ code: s.SupplierCode, label: `${s.SupplierCode} — ${s.SupplierName}` }))}
          ...
          <label className="flex flex-col gap-1 text-xs text-gray-500">
            เลขที่ใบส่งสินค้า
            <input
              value={form.supplierDeliveryNo}
              onChange={(e) => setForm({ ...form, supplierDeliveryNo: e.target.value })}
          ...
          <button
            ...
            disabled={pending || !form.warehouseCode || !form.supplierCode}
