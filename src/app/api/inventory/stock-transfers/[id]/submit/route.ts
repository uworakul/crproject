...
export async function POST(_req: Request, ctx: RouteContext<"/api/inventory/stock-transfers/[id]/submit">) {
  ...
  const existing = await prisma.invTransferHeader.findUnique({ where: { TransferHeaderID: transferId }, include: { Details: true } });
  if (!existing) return apiError(404, "STOCK_TRANSFER_NOT_FOUND");

  if (existing.Status !== "DRAFT") {
    return apiError(409, "INVALID_STATUS_TRANSITION", "Only a DRAFT transfer can be submitted", { currentStatus: existing.Status });
  }
  ...
  await logAction(user.userId, "SUBMIT_STOCK_TRANSFER", { targetTable: "inv_transfer_header", targetId: id });
