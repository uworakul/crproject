...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      if (clearFirst) {
        await tx.invProductCategory.deleteMany();
      }

      let created = 0;
      let updated = 0;
      for (const row of rows) {
        const existing = await tx.invProductCategory.findUnique({ where: { CategoryCode: row.code } });
        await tx.invProductCategory.upsert({
          where: { CategoryCode: row.code },
          create: { CategoryCode: row.code, CategoryName: row.name, CreatedBy: user.userId },
          update: { CategoryName: row.name, UpdatedBy: user.userId, UpdatedDate: new Date() },
        });
        if (existing) updated++;
        else created++;
      }
    ...
    await logAction(user.userId, "IMPORT_PRODUCT_CATEGORIES", {
      targetTable: "inv_product_category",
    ...
    return apiError(
      409,
      "CATEGORY_IN_USE",
      "Could not clear existing categories — one or more are linked to products and cannot be removed",
    );
  }
}
