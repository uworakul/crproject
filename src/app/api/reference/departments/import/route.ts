...
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      if (clearFirst) {
        await tx.refDepartment.deleteMany();
      }

      let created = 0;
      let updated = 0;
      for (const row of rows) {
        const existing = await tx.refDepartment.findUnique({ where: { DeptCode: row.code } });
        await tx.refDepartment.upsert({
          where: { DeptCode: row.code },
          create: { DeptCode: row.code, DeptName: row.name, CreatedBy: user.userId },
          update: { DeptName: row.name, UpdatedBy: user.userId, UpdatedDate: new Date() },
        });
        if (existing) updated++;
        else created++;
      }
    ...
    await logAction(user.userId, "IMPORT_DEPARTMENTS", {
      targetTable: "ref_department",
    ...
    return apiError(
      409,
      "DEPARTMENT_IN_USE",
      "Could not clear existing departments — one or more are linked to employees and cannot be removed",
    );
  }
}
